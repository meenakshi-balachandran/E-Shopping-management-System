...
function ProductDetail() {
  ...
  const data : ProductType = products.find(product => productId === String(product.id))
      ...
      <div className="flex items-start mt-16 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="md:flex-shrink px-4">
          <div className=" rounded-lg dark:bg-gray-700 mb-4">
            <img className="h-64 w-64 object-contain" src={data.image} alt="Product Image" />
          </div>
        </div>
        <div className="md:flex-1 px-4">
          ...
          <div className="flex -mx-2 mb-4">
            <div className="w-1/2 px-2">
              <button className="w-1/2 mt-20 bg-gray-900 dark:bg-gray-600 text-white py-2 px-4 rounded-full font-bold hover:bg-gray-800 dark:hover:bg-gray-700" onClick={() => addToCart(data)}>Add to Cart</button>
            </div>
          </div>
        </div>
      </div>
